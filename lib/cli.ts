...
import {
  compactIdToOutpoint,
  outpointToCompactId,
} from "./utils/atomical-format-helpers";
...
function printSuccess(data: any, showDonation?: boolean) {
  ...
  if (
    process.env.DISABLE_DONATE_QUOTE &&
    process.env.DISABLE_DONATE_QUOTE === "true"
  ) {
    return;
  }
  console.log(
  ...
  const donate =
    "bc1pl6k2z5ra403zfeyevzsu7llh0mdqqn4802p4uqfhz6l7qzddq2mqduqvc6";
  console.log(
    "Thank you for your support and contributions to Atomicals CLI development! ❤️"
  );
...
function showWalletFTBalancesDetails(obj: any, showutxos = false, accumulated) {
  const atomicalsUtxosByAtomicalId = groupAtomicalsUtxosByAtomicalId(
    obj.atomicals_utxos
  );
  for (const atomicalId in obj.atomicals_balances) {
    ...
    console.log(
      "Requested ticker status:",
      atomical["request_ticker_status"]["status"]
    );
    console.log("Ticker:", atomical["ticker"]);
    console.log("Confirmed balance:", atomical["confirmed"]);
    console.log(
      "UTXOs for Atomical:",
      atomicalsUtxosByAtomicalId[atomicalId].length
    );
    ...
    if (showutxos)
      console.log(
        JSON.stringify(atomicalsUtxosByAtomicalId[atomicalId], null, 2)
      );
  }
  return accumulated;
}
...
function showWalletDetails(
  ...
  if (showExtra) {
    ...
    for (const atomicalId in obj.atomicals_balances) {
      ...
      if (
        type !== "all" &&
        obj.atomicals_balances[atomicalId]["type"].toLowerCase() !== type
      ) {
        continue;
      }
...
function resolveWalletAliasNew(
...
program
  ...
  .description(
    "Creates and displays new 12-word secret mnemonic phrase along with the primary and funding addresses"
  )
  .action(async (options) => {
    ...
    console.log(
      `Primary address (P2TR): ${result.data.wallet.primary.address}`
    );
    ...
    console.log(
      `Funding address (P2TR): ${result.data.wallet.funding.address}`
    );
...
program
  .command("wallet-decode")
  .description(
    "Decode secret mnemonic phrase to display derive address and key at provided path"
  )
...
program
  ...
  .action(async (addressOrAlias, options) => {
    ...
    const result: { output: any; address: string; scripthash: string } =
      performAddressAliasReplacement(walletInfo, addressOrAlias || undefined);
...
program
  ...
  .action(async (address, options) => {
    try {
      ...
      const receive: { output: any; address: string } =
        performAddressAliasReplacement(walletInfo, address || undefined);
...
program
  ...
  .option(
    "--all",
    "Shows all loaded wallets and not just the primary and funding"
  )
  .option(
    "--extra",
    "Show extended wallet information such as specific utxos. Default is to only show a summary."
  )
  .option("--balances", "Show FT token balances")
  .option("--noqr", "Hide QR codes")
  .option(
    "--alias <string>",
    "Restrict to only showing one of the imported wallets identified by the alias"
  )
  .option(
    "--type <string>",
    "Show NFT or FT types only. By default shows both. Not compatible with --balances and --extra"
  )
  .option(
    "--identify <string>",
    "Restrict to only showing one of the imported wallets identified by the address (if it is found)"
  )
  .option(
    "--address <string>",
    "Show the data and a QR code for an arbitrary address. Not expected to be loaded into local wallets."
  )
  .action(async (options) => {
    try {
      ...
      const walletInfo = await validateWalletStorage();
      const config: ConfigurationInterface = validateCliInputs();
      const electrum = ElectrumApi.createClient(
      ...
      if (alias) {
        ...
        let result: any = await atomicals.walletInfo(
          walletInfo.imported[alias].address,
          history,
          keepElectrumAlive
        );
        console.log(
        ...
        console.log(
          `Wallet Information - ${alias} Address - ${result.data?.address}`
        );
        console.log(
          "========================================================================================================"
        );
        ...
        for (const walletAlias in walletInfo.imported) {
          ...
          if (walletInfo.imported[walletAlias].address === identify) {
            foundWallet = true;
            let result: any = await atomicals.walletInfo(
              walletInfo.imported[walletAlias].address,
              history,
              keepElectrumAlive
            );
            console.log(
        ...
        console.log(
          `Address Information - ${show} Address - ${result.data?.address}`
        );
        console.log(
          "========================================================================================================"
        );
        ...
        console.log(
          `1. Wallet Information - Primary Address - ${result.data?.address}`
        );
        console.log(
          "========================================================================================================"
        );
        ...
        result = await atomicals.walletInfo(
          walletInfo.funding.address,
          history,
          keepElectrumAlive
        );
        console.log(
        ...
        console.log(
          `2. Wallet Information - Funding Address - ${result.data?.address}`
        );
        console.log(
          "========================================================================================================"
        );
        ...
        if (all) {
          ...
          if (walletInfo.imported) {
            for (const walletAlias in walletInfo.imported) {
              ...
              console.log(
                `${counter}. Wallet Information - ${walletAlias} Address - ${result.data?.address}`
              );
              console.log(
                "========================================================================================================"
              );
...
program
  ...
  .option(
    "--address <string>",
    "Restrict to only showing by address. Using this option with --alias has no effect."
  )
  .action(async (options) => {
    try {
      const alias = options.alias ? options.alias : null;
      ...
      if (alias) {
        ...
        accumulated = showWalletFTBalancesDetails(
          result.data,
          utxos,
          accumulated
        );
...
program
  ...
  .action(async (container, limit, offset, options) => {
    try {
      ...
      const result: any = await atomicals.getContainerItems(
        container,
        parseInt(limit, 10),
        parseInt(offset, 10)
      );
...
program
  ...
  .action(async (container, itemId, options) => {
    try {
      ...
      const result: any = await atomicals.getAtomicalByContainerItem(
        modifiedStripped,
        itemId
      );
      console.log(JSON.stringify(result, null, 2));
...
program
  .command("validate-container-item")
  ...
  .action(async (containerName, itemName, manifestFile, options) => {
    try {
      ...
      const result: any = await atomicals.getAtomicalByContainerItemValidated(
        containerName,
        itemName,
        manifestFile
      );
...
program
  ...
  .action(async (atomicalAliasOrId, options) => {
    try {
      ...
      const verbose = options.verbose ? true : false;
      const modifiedStripped =
        atomicalAliasOrId.indexOf("+") === 0
          ? atomicalAliasOrId.substring(1)
          : atomicalAliasOrId;
...
program
  ...
  .action(async (options) => {
    try {
      ...
      let ownerWalletRecord = resolveWalletAliasNew(
        walletInfo,
        options.owner,
        walletInfo.primary
      );
      const filter = options.filter ? options.filter : undefined;
      const result: any = await atomicals.summarySubrealms(
        ownerWalletRecord.address,
        filter
      );
      handleResultLogging(result);
    } catch (error) {
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const filter = options.filter ? options.filter : undefined;
      const result: any = await atomicals.summaryContainers(
        ownerWalletRecord.address,
        filter
      );
      handleResultLogging(result);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const filter = options.filter ? options.filter : undefined;
      const result: any = await atomicals.summaryRealms(
        ownerWalletRecord.address,
        filter
      );
      handleResultLogging(result);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const filter = options.filter ? options.filter : undefined;
      const result: any = await atomicals.summaryTickers(
        ownerWalletRecord.address,
        filter
      );
      handleResultLogging(result);
...
program
  ...
  .option(
    "--funding <string>",
    "Use wallet alias WIF key to be used for funding"
  )
  ...
  .option(
    "--bitworkc <string>",
    "Whether to add any bitwork proof of work to the commit tx"
  )
...
program
  ...
  .action(async (containerName, jsonFilename, options) => {
    try {
      ...
      containerName = containerName.startsWith("#")
        ? containerName
        : "#" + containerName;
...
program
  ...
  .action(async (folder, outputFolderName) => {
    try {
      const result: any = await Atomicals.createDmintItemManifests(
        folder,
        outputFolderName
      );
      handleResultLogging(result);
...
program
  ...
  .action(async (folder, mintHeight, bitworkc) => {
    try {
      const result: any = await Atomicals.createDmint(
        folder,
        parseInt(mintHeight, 10),
        bitworkc
      );
      handleResultLogging(result);
    } catch (error) {
...
program
  .command("enable-dmint")
  .description(
    "Enable dmint for a container with the dmint config file produced from the prepare-dmint command"
  )
  .argument("<container>", "string")
  .argument("<jsonFilename>", "string")
  .option("--rbf", "Whether to enable RBF for transactions.")
...
program
  ...
  .option(
    "--owner <string>",
    "Owner of the parent Atomical. Used for direct subrealm minting."
  )
  .option(
    "--initialowner <string>",
    "Initial owner wallet alias to mint the Atomical into"
  )
  .option("--satsbyte <number>", "Satoshis per byte in fees", "15")
  .option(
    "--satsoutput <number>",
    "Satoshis to put into the minted atomical",
    "1000"
  )
  .option(
    "--funding <string>",
    "Use wallet alias WIF key to be used for funding and change"
  )
  .option("--container <string>", "Name of the container to request")
  .option(
    "--bitworkc <string>",
    "Whether to put any bitwork proof of work into the token mint. Applies to the commit transaction."
  )
  .option(
    "--bitworkr <string>",
    "Whether to put any bitwork proof of work into the token mint. Applies to the reveal transaction."
  )
  .option(
    "--disablechalk",
    "Whether to disable the real-time chalked logging of each hash for mining. Improvements mining performance to set this flag"
  )
  .action(async (containerName, itemName, manifestFile, options) => {
    try {
      const walletInfo = await validateWalletStorage();
      ...
      let initialOwnerAddress = resolveAddress(
        walletInfo,
      ...
      let fundingRecord = resolveWalletAliasNew(
        walletInfo,
        options.funding,
        walletInfo.funding
      );
...
program
  ...
  .option(
    "--bitworkr <string>",
    "Whether to add any bitwork proof of work to the reveal tx."
  )
  .option(
    "--disablechalk",
    "Whether to disable the real-time chalked logging of each hash for mining. Improvements mining performance to set this flag"
...
program
  .command("seal")
  .description(
    "Seal any NFT Atomical type permanently so that it can never be updated or transferred ever again."
  )
  .argument("<atomicalIdAlias>", "string")
  .option("--rbf", "Whether to enable RBF for transactions.")
  .option(
...
program
  ...
  .action(async (atomicalAliasOrId, options) => {
    try {
      ...
      const verbose = options.verbose ? true : false;
      const result = await atomicals.resolveAtomical(
        atomicalAliasOrId,
...
program
  ...
  .option(
    "--hashes <number>",
    "How many atomicals block hashes to retrieve",
    "10"
  )
  .action(async (options) => {
    try {
...
program
  ...
  .action(async (atomicalAliasOrId, options) => {
    try {
      ...
      const result = await atomicals.resolveAtomical(
        atomicalAliasOrId,
        AtomicalsGetFetchType.STATE_HISTORY,
        undefined,
        verbose
      );
      handleResultLogging(result);
...
program
  ...
  .action(async (atomicalAliasOrId, options) => {
    try {
      ...
      const result = await atomicals.resolveAtomical(
        atomicalAliasOrId,
        AtomicalsGetFetchType.EVENT_HISTORY,
        undefined,
        verbose
      );
      handleResultLogging(result);
...
program
  ...
  .action(async (realmOrSubRealm, file, options) => {
    try {
      ...
      const realmOrSubRealmAdded =
        realmOrSubRealm.indexOf("+") === 0
          ? realmOrSubRealm
          : "+" + realmOrSubRealm;
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const result = await atomicals.pendingSubrealms(
        options,
        ownerWalletRecord.address,
        fundingWalletRecord,
        satsbyte,
...
program
  ...
  .option(
    "--parent <string>",
    "Whether to require a parent atomical to be spent along with the mint."
  )
  .option(
    "--parentowner <string>",
    "Wallet owner of the parent to spend along with the mint."
  )
  .option(
    "--disablechalk",
    "Whether to disable the real-time chalked logging of each hash for mining. Improvements mining performance to set this flag"
  ...
  .action(async (ticker, supply, file, options) => {
    try {
      ...
      let parentOwnerRecord = resolveWalletAliasNew(
        walletInfo,
        options.parentowner,
        walletInfo.primary
      );
      ...
      let initialOwnerRecord = resolveAddress(
        walletInfo,
        options.initialowner,
        walletInfo.primary
      );
...
program
  .command("init-dft")
  .description(
    "Initialize fungible token (FT) atomical in decentralized issuance mode"
  )
  .argument("<ticker>", "string")
  ...
  .option(
    "--mintbitworkr <string>",
    "Whether to require any bitwork proof of work to mint. Applies to the reveal transaction."
  )
  .option(
    "--bitworkc <string>",
    "Whether to put any bitwork proof of work into the token mint. Applies to the commit transaction."
  ...
  .option(
    "--disablechalk",
    "Whether to disable the real-time chalked logging of each hash for Bitwork mining. Improvements mining performance to set this flag"
  )
  .action(
    async (
      ticker,
      ...
      try {
        ...
        let walletRecord = resolveWalletAliasNew(
          walletInfo,
          options.funding,
          walletInfo.funding
        );
...
program
  ...
  .option(
    "--container <string>",
    "Name of the container to request to be a member of. Not to be confused with the 'mint-container' command to create a new container"
  )
  .option(
    "--bitworkc <string>",
    "Whether to put any bitwork proof of work into the token mint. Applies to the commit transaction."
...
program
  ...
  .option(
    "--satsoutput <number>",
    "Satoshis to put into the transferred atomical",
    "546"
  )
  .action(async (atomicalId, address, options) => {
    try {
      ...
      const receive: { output: any; address: string } =
        performAddressAliasReplacement(walletInfo, address);
      const result = await atomicals.transferInteractiveNft(
        options,
        atomicalId,
        ownerWalletRecord,
        fundingWalletRecord,
...
program
  ...
  .option(
    "--atomicalreceipt <string>",
    "Attach an atomical id to a pay receipt"
  )
  .action(async (atomicalId, options) => {
    try {
      const walletInfo = await validateWalletStorage();
      ...
      const result = await atomicals.transferInteractiveFt(
        options,
        atomicalId,
        ownerWalletRecord,
        fundingWalletRecord,
...
program
  ...
  .option(
    "--skipvalidation",
    "Do not do FT transfer validation on broadcast (danger)"
  )
  .option(
    "--atomicalreceipt <string>",
    "Attach an atomical id to a pay receipt"
  ...
  .action(async (options) => {
    try {
      ...
      const result = await atomicals.transferInteractiveBuilder(
        options,
        ownerWalletRecord,
        fundingWalletRecord,
        walletInfo,
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const atomicalIdReceipt = options.atomicalreceipt;
      const result = await atomicals.transferInteractiveUtxos(
        options,
        ownerWalletRecord,
        fundingWalletRecord,
        walletInfo,
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const result = await atomicals.mergeInteractiveUtxos(
        options,
        ownerWalletRecord,
        fundingWalletRecord,
        walletInfo,
...
program
  ...
  .action(async (atomicalAliasOrId, options) => {
    try {
      ...
      const result = await atomicals.resolveAtomical(
        atomicalAliasOrId,
        AtomicalsGetFetchType.TX_HISTORY,
        undefined,
        verbose
      );
      handleResultLogging(result);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const result = await atomicals.list(
        parseInt(limit, 10),
        parseInt(offset, 10),
        asc
      );
...
program
  ...
  .option(
    "--body",
    "Whether to include the body bytes in the print out or suppress it"
  )
